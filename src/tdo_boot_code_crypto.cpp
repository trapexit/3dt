...
namespace
{
  ...
  {
    u32 key;
    const u64 key_offset = (word_offset_ % 16) * sizeof(key);
              ...
              u32 (*crypt_word_)(u32,u64))
  {
    ...
    if(((size_ % 4) != 0) || ((key_offset_ % 4) != 0))
      throw std::runtime_error("boot_code encrypted range is not 4-byte aligned");

    data = static_cast<char*>(data_);
    for(u64 pos = 0; pos < size_; pos += sizeof(u32))
      {
        u32 word;
        const u64 word_offset = (key_offset_ + pos) / sizeof(u32);

        memcpy(&word,&data[pos],sizeof(word));
        word = crypt_word_(word,word_offset);
        memcpy(&data[pos],&word,sizeof(word));
      }
