...
namespace
{
                   ...
                   const u64       size_)
  {
    ...
    if(!is)
      throw std::runtime_error("error opening " + filepath_.string());
    if(size_ > 0)
      is.read(data.data(),data.size());
    if(!is)
      throw std::runtime_error("error reading " + filepath_.string());
                         ...
                         rsa512_sig_t   sig_)
  {
    ...
    if(!is)
      throw std::runtime_error("error reading signature trailer from " + filepath_.string());
  }
                  ...
                  const rsa512_sig_t sig_)
  {
    ...
    if(!fs)
      throw std::runtime_error("error writing signature to " + filepath_.string());
  }
                         ...
                         const rsa512_sig_t sig_)
  {
    ...
    if(!os)
      throw std::runtime_error("error opening signature output " + filepath_.string());
    os.write((const char*)sig_,sizeof(rsa512_sig_t));
    if(!os)
      throw std::runtime_error("error writing signature output " + filepath_.string());
  }

  static
  u64
  signed_data_size(const fs::path &filepath_,
                   const bool     replace_,
                   const bool     verify_)
  {
    ...
    if(!replace_ && !verify_)
      return filesize;
    if(filesize < sizeof(rsa512_sig_t))
      throw std::runtime_error("file is too small to contain a signature trailer");

    return (filesize - sizeof(rsa512_sig_t));
  }
                ...
                const Options::SignFile &opts_)
  {
    ...
    const u64 filesize = fs::file_size(filepath_);
    const u64 data_size = signed_data_size(filepath_,opts_.replace,opts_.verify);
