...
namespace
{
  ...
  boot_code_romtag_size(const std::vector<char> &encrypted_data_)
  {
    ...
    signed_size = round_up(*executable_size,sizeof(u32)) + (RSA512_SIG_SIZE * 2);
  ...
  require_iso2048_image(TDO::FileStream &stream_)
  {
    if((stream_.device_block_header() != 0) ||
       ...
       (stream_.device_block_data_size() != PHY_BLOCK_SIZE))
      throw std::runtime_error("signing is currently supported only for 2048-byte ISO images");
  }

  static
  u64
  signature_file_size_for_digest_count(u64 num_digests_)
  {
    u64 digest_size;

    digest_size = num_digests_ * sizeof(md5_digest_t);
    ...
    return round_up(digest_size + RSA512_SIG_SIZE,PHY_BLOCK_SIZE);
  }

  static
  u64
  signature_record_size_for_digest_count(u64 num_digests_)
  {
  ...
  public:
               ...
               TDO::DevStream              &stream_)
    {
      ...
      update_record_sizes(stream_,
                          record_pos_,
                          romtags_file_size,
                          div_round_up(romtags_file_size,PHY_BLOCK_SIZE));
    }
  };
  ...
  public:
               ...
               TDO::DevStream              &stream_)
    {
      ...
      else if(include_banner_romtag && (lc_filepath == "bannerscreen"))
        type = RSA_APPSPLASH;
        ...
        case RSA_NEWKNEWNEWGNUBOOT:
          {
            ...
            if(!boot_size && ((record_.byte_count % sizeof(u32)) != 0))
              boot_size = round_up(record_.byte_count,sizeof(u32));
            apply_boot_romtag_version(romtag,
                                      boot_size.value_or(record_.byte_count),
                                      record_.byte_count);
  ...
  pad_image_and_update_disclabel(TDO::FileStream &stream_)
  {
    ...
    stream_.resize_multiple(LOG_BLOCK_SIZE);
                                 ...
                                 const SpecialFileCapacity &capacity_)
  {
    if(!capacity_.found_rom_tags)
      throw std::runtime_error("image is missing file: rom_tags");
    ...
    if(romtags_size > capacity_.rom_tags_capacity)
      throw std::runtime_error("rom_tags file too small, increase size and rebuild image");
    ...
    if(!capacity_.found_signatures)
      throw std::runtime_error("image is missing file: signatures");

    const u64 volume_block_count = stream_.disc_label().volume_block_count;
    const u64 num_digests = ((volume_block_count * PHY_BLOCK_SIZE) / LOG_BLOCK_SIZE);
    const u64 file_size = signature_file_size_for_digest_count(num_digests);
    const u64 record_size = signature_record_size_for_digest_count(num_digests);
                                  ...
                                  const SpecialFileCapacity &capacity_)
  {
    ...
    const u64 record_size =
      std::max<u64>(capacity_.signatures_record_size,
                    signature_record_size_for_digest_count(num_digests));
                          ...
                          const bool       include_banner_romtag_)
  {
    ...
    if(!preflight.found_boot_code)
      throw std::runtime_error("image is missing file: system/kernel/boot_code");
    if(!preflight.found_os_code)
      throw std::runtime_error("image is missing file: system/kernel/os_code");
    if(!preflight.found_misc_code)
      throw std::runtime_error("image is missing file: system/kernel/misc_code");

    const u64 padded_size = round_up(stream_.size_in_device_blocks() * PHY_BLOCK_SIZE,
                                     LOG_BLOCK_SIZE);
    const u64 volume_block_count = padded_size / PHY_BLOCK_SIZE;
    const u64 num_digests = ((volume_block_count * PHY_BLOCK_SIZE) / LOG_BLOCK_SIZE);
    const u64 signature_size = signature_file_size_for_digest_count(num_digests);
                      ...
                      const char      *label_)
  {
    ...
    if(romtag->size < RSA512_SIG_SIZE)
      throw std::runtime_error(std::string(label_) + " is too small to contain a signature");
    ...
    stream_.data_byte_seek(((romtag->offset + 1) * PHY_BLOCK_SIZE) +
                           (romtag->size - RSA512_SIG_SIZE));
  ...
  sign_boot_code(TDO::FileStream &stream_)
  {
    ...
    if(romtag->size < (RSA512_SIG_SIZE * 2))
      throw std::runtime_error("boot_code is too small to contain both signatures");
    ...
    encrypted_sig.assign(sig,sig + sizeof(sig));
  ...
  generate_signatures_file_data(TDO::FileStream &stream_)
  {
    ...
    num_digests        = ((volume_block_count * volume_block_size) / LOG_BLOCK_SIZE);
    ...
    for(u64 i = 0; i < num_digests; i++)
      {
        ...
        buf.clear();
        stream_.read_data_blocks(buf,block_pos,(LOG_BLOCK_SIZE / PHY_BLOCK_SIZE));
                               ...
                               const u32        signatures_record_size_)
  {
    ...
    if(!found)
      throw std::runtime_error("signatures ROM tag not found");
  }
  ...
  signature_file_capacity(TDO::FileStream &stream_)
  {
    ...
    if(err)
      throw std::runtime_error(err.str);
    if(!capacity.found)
      throw std::runtime_error("signatures file not found");
                         ...
                         const u64          byte_count_)
  {
    ...
    const u64 start_byte = start_block_ * PHY_BLOCK_SIZE;
    const u64 end_byte = start_byte + byte_count_;
    const u64 first_digest = start_byte / LOG_BLOCK_SIZE;
    const u64 last_digest = (end_byte - 1) / LOG_BLOCK_SIZE;
  ...
  recreate_layout_signatures_file(TDO::FileStream &stream_)
  {
    ...
    num_digests = ((stream_.disc_label().volume_block_count * PHY_BLOCK_SIZE) / LOG_BLOCK_SIZE);
    file_size = std::max<u64>(signature_file_size_for_digest_count(num_digests),
                              sig_romtag->size);
    ...
    signatures.resize(file_size - sizeof(sig));
    ...
    signatures.insert(signatures.end(),sig,sig + sizeof(sig));

    const s64 sig_offset = sig_romtag->offset + 1;
    for(u64 i = 0; i < div_round_up(signatures.size(),PHY_BLOCK_SIZE); i++)
      {
        const u64 offset = i * PHY_BLOCK_SIZE;
        const u64 bytes_left = signatures.size() - offset;
        const u64 bytes_to_write = std::min<u64>(PHY_BLOCK_SIZE,bytes_left);
  ...
  resize_signatures_file_record(TDO::FileStream &stream_)
  {
    ...
    num_digests = ((stream_.disc_label().volume_block_count * PHY_BLOCK_SIZE) / LOG_BLOCK_SIZE);
    file_size = signature_file_size_for_digest_count(num_digests);
    ...
    sfu.signatures_block_count = div_round_up(file_size,PHY_BLOCK_SIZE);
  ...
  sign_disclabel_romtags_bootcode(TDO::FileStream &stream_)
  {
    ...
    romtag = stream_.romtag(RSA_NEWKNEWNEWGNUBOOT);
    if(!romtag)
      throw std::runtime_error("boot_code ROM tag not found");
  ...
  try
    {
      ...
      err = stream.open(filepath_,std::ios::in|std::ios::out);
      if(err)
        return err;
