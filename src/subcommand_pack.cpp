...
namespace
{
  ...
  static constexpr u32 LOG_BLOCK_SIZE   = 32768;
  ...
  static
  u32
  checked_u32(u64 value_,
              const char *label_)
  {
    if(value_ > std::numeric_limits<u32>::max())
      throw std::runtime_error(std::string(label_) + " is too large");
  ...
  block_count_for_size(u64 size_,
                       u32 block_size_ = BLOCK_SIZE)
  {
    if(size_ == 0)
      return 0;
    if(block_size_ == 0)
      throw std::runtime_error("block size must be non-zero");

    return checked_u32(((size_ + block_size_ - 1) / block_size_),"block count");
  }
  ...
  physical_block_count(const Entry &entry_)
  {
    ...
    return checked_u32(((bytes + BLOCK_SIZE - 1) / BLOCK_SIZE),
  ...
  {
    if(name_.empty())
      throw std::runtime_error("empty filename is not supported");
    if(name_.size() >= FILESYSTEM_MAX_NAME_LEN)
      throw std::runtime_error("filename is too long for OperaFS: " + name_);
  }
  ...
  make_entry(const fs::directory_entry &dirent_)
  {
    ...
    if(entry->directory)
    ...
    else
      {
        ...
        size = fs::file_size(dirent_.path());
        entry->byte_count      = checked_u32(size,"file size");
                 ...
                 bool           root_ = true)
  {
    ...
    for(const auto &dirent : fs::directory_iterator(path_))
      {
        ...
        if(!is_supported_source(dirent))
          throw std::runtime_error("unsupported filesystem entry: " + dirent.path().string());
    ...
    std::sort(dirents.begin(),
              ...
              {
                return (lhs_.path().filename().string() < rhs_.path().filename().string());
              });
            ...
            const char        *label_)
  {
    ...
    if((end == str_.c_str()) || (*end != '\0'))
      throw std::runtime_error(std::string("invalid layout ") + label_ + ": " + str_);
    if(value > std::numeric_limits<u32>::max())
      throw std::runtime_error(std::string("layout ") + label_ + " is too large");
                          ...
                          const json        &layout_)
  {
    ...
    manifest_.disc_label.record_type = json_u32_value(label,"record_type",RECORD_STD_VOLUME);
    ...
    if(label.contains("volume_sync_bytes"))
      {
        ...
        for(std::size_t i = 0; (i < values.size()) && (i < manifest_.disc_label.volume_sync_bytes.size()); i++)
          manifest_.disc_label.volume_sync_bytes[i] = values[i];
      }
    manifest_.disc_label.volume_structure_version = json_u32_value(label,"volume_structure_version",VOLUME_STRUCTURE_OPERA_READONLY);
    manifest_.disc_label.volume_flags = json_u32_value(label,"volume_flags",0);
    ...
    manifest_.disc_label.volume_block_size = json_u32_value(label,"volume_block_size",BLOCK_SIZE);
    ...
    manifest_.disc_label.root_directory_block_size = json_u32_value(label,"root_directory_block_size",BLOCK_SIZE);
    ...
    if(manifest_.disc_label.root_directory_last_avatar_index >= manifest_.disc_label.root_directory_avatar_list.size())
      throw std::runtime_error("layout root directory avatar list is too large");
    ...
    manifest_.root.byte_count = manifest_.root.block_count * manifest_.root.block_size;
              ...
              TDO::DiscManifest &manifest_)
  {
    ...
    is.open(layout_);
    if(!is)
      throw std::runtime_error("failed to open layout file: " + layout_.string());

    is >> layout;
    if(layout.value("format",std::string()) != "3dt-operafs-layout")
      throw std::runtime_error("unsupported layout manifest format");
    if(layout.contains("image") &&
       (layout["image"].value("device_block_data_size",BLOCK_SIZE) != BLOCK_SIZE))
      throw std::runtime_error("packing NVRAM or other non-2048-byte images is not supported yet");
    ...
    for(const auto &entry_json : layout.at("entries"))
      {
        ...
        record.block_size         = json_u32_value(entry_json,"block_size",BLOCK_SIZE);
               ...
               const LayoutMap &layout_)
  {
    ...
    std::sort(children.begin(),
              children.end(),
              [&](const Entry::Ptr &lhs_,
                  const Entry::Ptr &rhs_)
              {
                u32 lhs_order;
                u32 rhs_order;

                lhs_order = layout_order(*lhs_,entry_path_ / lhs_->name,layout_);
                rhs_order = layout_order(*rhs_,entry_path_ / rhs_->name,layout_);
  ...
  static
  u32
  compute_directory_block_count(const Entry &entry_);
                             ...
                             const fs::path &entry_path_)
  {
    if((entry_.block_count > 0) && (entry_.block_size == 0))
      throw std::runtime_error("layout entry has zero block size: " +
    ...
    if(entry_.directory)
      {
        ...
        required_blocks = compute_directory_block_count(entry_);
        if(entry_.block_count < required_blocks)
          throw std::runtime_error("layout directory allocation too small: " +
                                   display_path(entry_path_) + " needs " +
                                   std::to_string(required_blocks) +
                                   " blocks but has " +
                                   std::to_string(entry_.block_count));
        entry_.byte_count = checked_u32(static_cast<u64>(entry_.block_count) * entry_.block_size,
    ...
    else if(entry_.kind == EntryKind::Normal)
      {
        ...
        if(entry_.src_path.empty())
          throw std::runtime_error("layout entry has no source file: " + entry_path_.generic_string());

        source_size = fs::file_size(entry_.src_path);
        capacity = static_cast<u64>(entry_.block_count) * entry_.block_size;
        if(source_size > capacity)
          throw std::runtime_error("layout byte count exceeds allocation: " +
                                   display_path(entry_path_));
        required_blocks = block_count_for_size(source_size,entry_.block_size);
        if(entry_.block_count < required_blocks)
          throw std::runtime_error("layout file allocation too small: " +
                                   display_path(entry_path_) + " needs " +
                                   std::to_string(required_blocks) +
                                   " blocks but has " +
                                   std::to_string(entry_.block_count));

        entry_.byte_count = checked_u32(source_size,"file byte count");
  ...
  compute_directory_sizes(Entry &entry_)
  {
    ...
    entry_.block_count = compute_directory_block_count(entry_);
    entry_.byte_count  = entry_.block_count * BLOCK_SIZE;
  ...
  signature_digest_count_for_volume(u32 volume_blocks_)
  {
    ...
    padded_blocks   = round_up(volume_blocks_,(LOG_BLOCK_SIZE / BLOCK_SIZE));

    return checked_u32((padded_blocks * BLOCK_SIZE) / LOG_BLOCK_SIZE,"signature digest count");
  }

  static
  u32
  signature_file_size_for_digest_count(u32 num_digests_)
  {
    u64 digest_size;
    u64 signatures_size;

    digest_size = static_cast<u64>(num_digests_) * sizeof(md5_digest_t);
    ...
    if((num_digests_ & 511) == 0)
      signatures_size = digest_size + 8192;
    else
      signatures_size = round_up(digest_size + RSA512_SIG_SIZE,BLOCK_SIZE);

    return checked_u32(signatures_size,"signatures file size");
  }

  static
  u32
  signature_record_size_for_digest_count(u32 num_digests_)
  {
    ...
    if((num_digests_ & 511) == 0)
      return checked_u32(static_cast<u64>(num_digests_) * sizeof(md5_digest_t),
  ...
  {
    return signature_file_size_for_digest_count(signature_digest_count_for_volume(volume_blocks_));
  }
  ...
  {
    return checked_u32(required_signature_file_size_for_volume(volume_blocks_) / BLOCK_SIZE,
                          ...
                          u32    volume_blocks_ = 0)
  {
    ...
    if(entry == nullptr)
      throw std::runtime_error("layout removed required signatures file");

    if(volume_blocks_ != 0)
      {
        num_digests = signature_digest_count_for_volume(volume_blocks_);
        sig_blocks = checked_u32(signature_file_size_for_digest_count(num_digests) / BLOCK_SIZE,
                                 "signatures file block count");
        record_size = signature_record_size_for_digest_count(num_digests);
      }
    else
      {
        base_blocks = FIRST_FILE_BLOCK;
        base_blocks += sum_directory_blocks(root_);
        base_blocks += sum_file_blocks_without_signatures(root_);
        sig_blocks = required_signature_blocks(base_blocks);
        num_digests = signature_digest_count_for_volume(base_blocks + sig_blocks);
        record_size = signature_record_size_for_digest_count(num_digests);
      }
  ...
  max_allocated_block(const Entry &entry_)
  {
    ...
    for(auto avatar : entry_.avatar_list)
      max_block = std::max(max_block,
                           checked_u32(static_cast<u64>(avatar) + physical_blocks,
                                       "allocated block"));
    if(entry_.avatar_list.empty() && (entry_.block_count > 0))
      max_block = std::max(max_block,
                           checked_u32(static_cast<u64>(entry_.start_block) + physical_blocks,
                                       "allocated block"));
    for(const auto &child : entry_.children)
    ...
    if(key == "disc label")
      {
        if((entry_.block_count != 1) || avatars_.empty())
          throw std::runtime_error("layout cannot resize special file: " +
                                   display_path(entry_path_));
        if(avatars_[0] != 0)
          throw std::runtime_error("layout cannot relocate Disc label; it must use block 0");
      }
    else if(key == "rom_tags")
      {
        if((entry_.block_count > 1) || avatars_.empty())
          throw std::runtime_error("layout cannot resize special file: " +
                                   display_path(entry_path_));
        if(avatars_[0] != 1)
          throw std::runtime_error("layout cannot relocate rom_tags; it must use block 1");
      }
  }
                      ...
                      bool                        implicit_)
  {
    ...
    if((end_block < start_block_) || (end_block > std::numeric_limits<u32>::max()))
      throw std::runtime_error("layout allocation is too large: " + label_);
        ...
        for(std::size_t j = i; j > 0; j--)
          {
            ...
            throw std::runtime_error("layout block overlap: " +
    ...
    if(entry_.block_count > 0)
      {
        ...
        next_block_ += entry_.block_count;
      }
                           ...
                           const fs::path &output_)
  {
    ...
    input_abs  = fs::weakly_canonical(input_);
    output_abs = fs::absolute(output_).lexically_normal();
    ...
    if(input_it == input_abs.end())
      throw std::runtime_error("output image cannot be inside the input directory");
  }
  ...
  layout_path_for(const Options::Pack &options_)
  {
    ...
    if(!fs::is_regular_file(layout))
      throw std::runtime_error("layout path is not a regular file: " + layout.string());
  ...
  static
  fs::path
  temp_output_path_for(const fs::path &path_)
  {
  ...
  create_manifest(const Options::Pack &options_)
  {
    ...
    if(options_.volume_commentary.size() >= VOLUME_COM_LEN)
      throw std::runtime_error("volume commentary is too long for OperaFS");
    ...
    if(options_.volume_label.size() >= VOLUME_ID_LEN)
      throw std::runtime_error("volume label is too long for OperaFS");
    memcpy(&manifest.disc_label.volume_identifier[0],
           options_.volume_label.c_str(),
           options_.volume_label.size());
    manifest.disc_label.volume_block_size = BLOCK_SIZE;
    manifest.disc_label.root_directory_block_size = BLOCK_SIZE;
    ...
    manifest.root.block_size = BLOCK_SIZE;
    ...
    add_or_replace_synthetic_entries(manifest.root,
                                     (options_.sign ||
                                      (layout.find("signatures") != layout.end())));
...
namespace Subcommand
{
  ...
  pack(const Options::Pack &options_)
  {
    ...
    if(options_.dry_run)
      {
        fmt::print("{}:\n"
                   ...
                   options_.output,
                   manifest.total_blocks,
                   static_cast<u64>(manifest.total_blocks) * BLOCK_SIZE);
        return;
      }
    ...
    err = TDO::pack_disc_image(manifest);
    if(err)
      {
        std::error_code ec;
    ...
    try
      {
        ...
        if(options_.mark)
          {
            err = TDO::mark_disc_image(temp_output_path,
                                       (options_.sign ?
                                        "packed and signed" :
                                        "packed"));
            if(err)
        ...
        if(recreate_layout_specials)
          {
            err = TDO::recreate_layout_special_files(temp_output_path,
                                                     options_.sign,
                                                     false,
        ...
        if(options_.sign && !recreate_layout_specials)
          {
            err = TDO::sign_disc_image(temp_output_path,
                                       false,
                                       true,
                                       options_.banner_romtag,
                                       digest_check_count);
