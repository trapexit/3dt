...
namespace
{
  ...
  physical_block_count(const Entry &entry_)
  {
    ...
    return checked_u32(((bytes + TDO::BLOCK_SIZE - 1) / TDO::BLOCK_SIZE),
  ...
  validate_filename(const std::string &name_)
  {
    ...
    if(name_.size() >= FILESYSTEM_MAX_NAME_LEN)
      throw Error("filename is too long for OperaFS: " + name_);
  }
  ...
  compute_directory_sizes(Entry &entry_)
  {
    ...
    entry_.byte_count  = entry_.block_count * TDO::BLOCK_SIZE;
  ...
  signature_digest_count_for_volume(u32 volume_blocks_)
  {
    ...
    return checked_u32((padded_blocks * TDO::BLOCK_SIZE) / TDO::LOG_BLOCK_SIZE,"signature digest count");
  }
  ...
  {
    return checked_u32(TDO::signature_file_size_for_digest_count(signature_digest_count_for_volume(volume_blocks_)),
  ...
  {
    return checked_u32(required_signature_file_size_for_volume(volume_blocks_) / TDO::BLOCK_SIZE,
    ...
    if(volume_blocks_ != 0)
      {
        ...
        sig_blocks = checked_u32(TDO::signature_file_size_for_digest_count(num_digests) / TDO::BLOCK_SIZE,
                                 "signatures file block count");
