...
**Note:** The original Portfolio OS code also references `PUBLIC_SIGNATURES_ARRAY` (a 432-byte ASCII credits message), but that pointer is overwritten before the first dereference and every 16 words thereafter. In practice, only `BOOT_CODE_CRYPT_KEY` (equivalent to `PUBLIC_THDOKEY_MOD` without the leading zero) is used.
...
This algorithm originates from the Portfolio OS source code, specifically `portfolio_os/src/dipir/rsadipir.c`. It was used internally by 3DO developers for various obfuscation purposes during the platform's development and publishing workflow.
